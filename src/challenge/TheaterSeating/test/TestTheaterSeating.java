...
public class TestTheaterSeating {
	...
	private void checkFirstCustomerSeat() {
		...
		if (testTheater.getResults().get("R001").equals(list)) {
			System.out
					.println("Test 1 Passed : 5 Seats successfully reserved for first customer at the middle row.");
		} else {
			System.out
					.println("Test 1 failed: Failed to reserve seats for first customer at the middle row.");
		}
	}
	...
	public void checkConsecutiveSeats() {
		...
		if (testTheater.getList(4, 0, 4).equals(list)) {
			System.out
					.println("Test 2 Passed : 5 Consecutive seats successfully reserved for first customer row E.");

		} else {
			System.out
					.println("Test 2 failed : Failed to reserve consecutive seats.");
		}
	...
	public void checkInsufficientSeats() {
		...
		if (testTheater.getNumberOfSeats() > 0) {
			System.out
					.println("Test 3 Passed : Failed to allocate seats when the request was greater than the available seats.");
		} else {
			System.out
					.println(" Test 3 Failed : Allocated as many seta as possible.");
		}
	}
	...
	public void checkGroupUnableToAccomodateInRow() {
		...
		if (result == 0) {
			System.out
					.println("Test 4 Passed : Successfully allocated seats to a large group that could not be accomodated in a row.");
		} else {
			System.out
					.println("Test 4 Failed : Failed to allocate seats to a large group.");
		}
	}
}
